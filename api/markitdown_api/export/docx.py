...
def build_docx(pages: list[dict[str, Any]], title: str = "OCR Export") -> bytes:
    ...
    if any(page_lines):
        doc = _build_layout(pages_by_number, page_lines)
    else:
        doc = _build_plain(pages_by_number, title)
...
def _build_layout(
    pages_by_number: list[dict[str, Any]], page_lines: list[list["_Line"]]
) -> Document:
    doc = Document()
...
def _build_plain(pages_by_number: list[dict[str, Any]], title: str) -> Document:
    doc = Document()
